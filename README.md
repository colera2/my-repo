...
네이버·쿠팡 광고 보고서 파일을 웹사이트에 업로드하면, 자동으로 데이터베이스에 저장해주는 시스템입니다.  
...
- 광고 보고서 파일(`.xlsx`, `.csv`) 웹 업로드
- **파일명 기반 자동 판별** — 파일명만 보고 광고 종류를 자동으로 구분하여 처리
...
DB 접속 정보 등 민감한 정보는 **코드에 직접 입력하지 말고** 환경 변수로 관리해야 합니다.

Azure Static Web Apps의 [애플리케이션 설정] 또는 로컬의 `api/local.settings.json`에 아래 항목을 설정하세요.
...
> ⚠️ `local.settings.json`은 `.gitignore`에 반드시 추가하여 GitHub에 올라가지 않도록 주의하세요.
